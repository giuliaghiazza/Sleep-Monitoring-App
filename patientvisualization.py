...
class AppointmentPage(ctk.CTkFrame):
    ...
    def setup_gui(self): 
        ...
        self.cursor.execute('SELECT DISTINCT slot_tempo FROM appointment WHERE dispo=1')
        ...
        confirm_button = ctk.CTkButton(
            ...
            width=80,
            font=ctk.CTkFont(size=16),
            command=lambda: self.conferma(slot_sel, doc_sel)
        )
        ...
        self.cursor.execute('SELECT n_booked FROM patients WHERE nome_utente=?', (user,))
        n_booked_tup= self.cursor.fetchone()
        n_booked = int(n_booked_tup[0])
        n_book=len(n_booked_tup)
        self.cursor.execute('SELECT codice_appuntamento, slot_tempo, doctor FROM appointment WHERE patient=?', (user,))
        ...
        self.cursor.execute('SELECT slot_tempo FROM appointment WHERE patient=?', (user,))
        ...
        self.combobox_1.set("")
        self.cursor.execute('SELECT doctor FROM appointment WHERE patient=?', (user,))
        ...
        if len(visit_booked)>0:
            for i in range(n_book):
                booked_round = f"Codice appuntamento: {visit_booked[i][0]}, Orario: {visit_booked[i][1]}, Dottore: {visit_booked[i][2]}"
    ...
    def conferma(self, slot_sel, doc_sel, user):
        self.cursor.execute('SELECT codice_appuntamento FROM appointment WHERE slot_tempo=?  AND doctor=?', slot_sel, doc_sel)
        codice=self.cursor.fetchone()
        self.cursor.execute('UPDATE appointment (patient, dispo) VALUES(?, 0) WHERE codice_appuntamento = ?;', user, codice)              #con idea che parto con tabella gia piena con slot ora e dottore, ma dispo=1
    ...
    def get_selected_text(self):
        ...
        if self.selected_index is not None:
            ...
            self.cursor.execute('SELECT DISTINCT doctor FROM appointment WHERE dispo= 1 AND slot_tempo=?',slot_sel)
    ...
    def select_frame_by_name(self, name):
        ...
        if name == "Manage Appointment":
            self.controller.show_page("appointment")
            self.close_sidebar()
        if name == "Health data record":
            self.controller.show_page("data")
            self.close_sidebar()
        if name == "Emergency communications":
            self.controller.show_page("emergency")
            self.close_sidebar()
...
class EmergencyPage(ctk.CTkFrame):
    ...
    def setup_gui(self): 
        ...
        self.cursor.execute('SELECT Name FROM patients WHERE nome_utente=?',(user,))         #anche qua va capito come passarlo poi in implementazione reale
        ...
        self.cursor.execute('SELECT Surname FROM patients WHERE nome_utente=?',(user,))
        ...
        self.cursor.execute('SELECT Codice_Fiscale FROM patients WHERE nome_utente=?',(user,))
        ...
        self.cursor.execute('SELECT City_of_Recidency FROM patients WHERE nome_utente=?',(user,))
        ...
        self.cursor.execute('SELECT Province_of_Recidency FROM patients WHERE nome_utente=?',(user,))
    ...
    def login_callback(self):
        ...
        if result is not None:
            ...
            if pw==password_try:
                ...
                role = self.cursor.execute('SELECT role FROM patients WHERE nome_utente=?', user)
                if role=='Doctor':
...
class SigninPage(ctk.CTkFrame):
    ...
    def signin_gui(self):
        ...
        signin_button = ctk.CTkButton(
            master=self,
            text="Next",     #da aggiungere icona se vogliamo metterne una
            height=50,
            width=40,
            font=ctk.CTkFont(size=16),
            #command=self.signin_callback
            command=lambda: self.controller.show_page("Sign-in_1")
        )
        signin_button.grid(row=5, column=0, padx=115, pady=10, sticky="ew")
    ...
    def signin_callback(self):
        ...
        if user is not None:
            if pw is not None:
                self.cursor.execute('INSERT INTO patients(nome_utente, psw) VALUES (?,?)', user, pw)
                result = self.cursor.fetchone()
            if result is not None:
               self.cursor.execute('SELECT psw FROM patients WHERE nome_utente=?', user)
               password_try= self.cursor.fetchone()[0]
...
class PatientVisualization():
    ...
    def __init__(self):      # When launching the app
        ...
        self.show_page("emergency")   #prima era home, se non funziona torno qua
        self.root.mainloop()       
